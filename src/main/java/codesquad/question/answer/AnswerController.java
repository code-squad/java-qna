...
public class AnswerController {
    ...
    public String post(@PathVariable Long questionId, @PathVariable Long id, HttpSession session, String comment){
        ...
        answer.setQuestion(questionRepository.findById(questionId).orElseThrow(() -> new QuestionNotFoundException("해당 질문을 찾을 수 없습니다.")));
        answer.setUser(userRepository.findById(id).orElseThrow(() -> new UserNotFoundException("해당 유저를 찾을 수 없습니다.")));
        answer.setTime(Time.getTodayDate());
